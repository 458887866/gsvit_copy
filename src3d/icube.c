...
SvICube*
sv_icube_new(gint xres, gint yres, gint zres, gdouble xreal, gdouble yreal, gdouble zreal, gboolean nullme)
{
    gint i, j;
...
SvICube*
sv_icube_new_alike(SvICube *icube, gboolean nullme)
{
    return sv_icube_new(icube->xres, icube->yres, icube->zres, icube->xreal, icube->yreal, icube->zreal, nullme);
...
void
sv_icube_fill(SvICube *icube, gint value)
{
    gint i, j, k;
...
void
sv_icube_add(SvICube *icube, gint value)
{
    gint i, j, k;
    //#pragma omp parallel default(shared) private(j, k)
    for (i = 0; i < icube->xres; i++) {
...
void
sv_icube_multiply(SvICube *icube, gint value)
{
    gint i, j, k;
    //#pragma omp parallel default(shared) private(j, k)
    for (i = 0; i < icube->xres; i++) {
...
void
sv_icube_get_datafield(SvICube *icube, GwyDataField *dfield, gint ipos, gint jpos, gint kpos)
{
    gint i, j, k;
    if (ipos == -1 && jpos == -1) {
