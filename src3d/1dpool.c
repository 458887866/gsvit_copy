...
{
    jp->e->data[0] = jp->fbnx0_e_1 + (jp->dt*LIGHT_SPEED-jp->dx) / (jp->dt*LIGHT_SPEED+jp->dx) * (jp->e->data[1] - jp->fbnx0_e_0);

    jp->h->data[0] = jp->fbnx0_h_1 + (jp->dt*LIGHT_SPEED-jp->dx) / (jp->dt*LIGHT_SPEED+jp->dx) * (jp->h->data[1] - jp->fbnx0_h_0);

    jp->e->data[jp->res-1] = jp->fbnxn_e_1 + (jp->dt*LIGHT_SPEED-jp->dx) / (jp->dt*LIGHT_SPEED+jp->dx) * (jp->e->data[jp->res-2] - jp->fbnxn_e_0);

    jp->h->data[jp->res-1]=jp->fbnxn_h_1 + (jp->dt*LIGHT_SPEED-jp->dx) / (jp->dt*LIGHT_SPEED+jp->dx) * (jp->h->data[jp->res-2] - jp->fbnxn_h_0);
}
