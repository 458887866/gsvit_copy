...
SvFCube *sv_fcube_new(gint xres, gint yres, gint zres,
                      gfloat xreal, gfloat yreal, gfloat zreal,
                      gboolean nullme)
{
    gint i, j;
...
{
    return sv_fcube_new(fcube->xres, fcube->yres, fcube->zres,
                        fcube->xreal, fcube->yreal, fcube->zreal, nullme);
}
...
void sv_fcube_get_datafield   (SvFCube *fcube, GwyDataField *dfield, gint ipos, gint jpos, gint kpos)
{
    gint i, j, k;
    if (ipos==-1 && jpos==-1)
    {
