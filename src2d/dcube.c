...
{
    return sv_dcube_new(dcube->xres, dcube->yres, dcube->zres,
                        dcube->xreal, dcube->yreal, dcube->zreal, nullme);
}
...
void sv_dcube_get_datafield   (SvDCube *dcube, GwyDataField *dfield, gint ipos, gint jpos, gint kpos)
{
    gint i, j, k;
    if (ipos==-1 && jpos==-1)
    {
